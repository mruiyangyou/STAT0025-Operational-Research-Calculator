...
class MarkovDp(object):
    ...
    def get_reward_df(self, n, alpha):
        if not isinstance(n, (float, int)) or not isinstance(alpha, (float, int)) or alpha > 1 or alpha < 0:
            raise ValueError('N and alpha must be float or interger, alpha must between 0 and 1!')
